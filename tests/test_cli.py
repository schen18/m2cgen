...
def test_generate_code(pickled_model):
    ...
    verify_python_model_is_expected(
        ...
        expected_output=11089.853508081202)
...
def test_joblib_loading(pickled_model):
    mock_args = _get_mock_args(infile=pickled_model, language="rust", lib="joblib")
    generated_code = cli.generate_code(mock_args).strip()

    assert generated_code.startswith("fn score(input: Vec<f64>) -> f64 {")


def test_sql_language(pickled_model):
    mock_args = _get_mock_args(infile=pickled_model, language="sql")
    generated_code = cli.generate_code(mock_args).strip()

    assert generated_code.startswith(
        "CREATE OR REPLACE MACRO score(feature_0, feature_1, feature_2,")


def test_sql_dialect(pickled_model):
    mock_args = _get_mock_args(
        infile=pickled_model, language="sql", dialect="mysql")
    generated_code = cli.generate_code(mock_args).strip()

    assert generated_code.startswith("CREATE FUNCTION `score`")
