...
from m2cgen.interpreters.sql import resolve_feature_names
...
def export_to_sql(model, dialect="duckdb", indent=4, function_name="score",
                  feature_names=None):
    """
    Generates a SQL code representation of the given model.

    The model is emitted as a DuckDB macro (`dialect="duckdb"`, the
    default), a MySQL/MariaDB stored function (`dialect="mysql"`) or a
    PostgreSQL PL/pgSQL function (`dialect="postgres"`). Features are
    exposed as function/macro parameters, so the generated code can be
    used with the caller's own column names:

        SELECT score(col_a, col_b) FROM some_table

    When `feature_names` is not specified, they are resolved from the
    model itself (models trained on pandas DataFrames carry feature
    names); otherwise positional names (`feature_0`, ...) are used.
    ...
    dialect : string, optional
        SQL dialect: "duckdb" (default), "mysql" or "postgres".
    indent : int, optional
        The size of indents in the generated code.
    function_name : string, optional
        Name of the function in the generated code.
    feature_names : list of strings, optional
        Names of the features; used as the parameter names.

    Returns
    -------
    code : string
    """
    interpreter = interpreters.SqlInterpreter(
        dialect=dialect,
        indent=indent,
        function_name=function_name,
        feature_names=feature_names
        if feature_names is not None else resolve_feature_names(model))
    return _export(model, interpreter)
