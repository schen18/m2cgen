...
class GLMMixin:
    ...
    def _logc_inversed(self, ast_to_transform):
        # g(p) = log(1 - p)  =>  g^-1(z) = 1 - exp(z)
        return utils.sub(
            ast.NumVal(1.0),
            ast.ExpExpr(ast_to_transform))

    def _loglog_inversed(self, ast_to_transform):
        # g(p) = -log(-log(p))  =>  g^-1(z) = exp(-exp(-z))
        return ast.ExpExpr(
            utils.mul(
                ast.NumVal(-1.0),
                ast.ExpExpr(
                    utils.mul(ast.NumVal(-1.0), ast_to_transform))))
...
class StatsmodelsGLMModelAssembler(GLMMixin, StatsmodelsLinearModelAssembler):
    ...
    def _get_supported_inversed_funs(self):
        return {
            ...
            "inversepower": self._inverse_power_inversed,  # class name since statsmodels 0.14
            "sqrt": self._sqrt_inversed,
            "inverse_squared": self._inverse_squared_inversed,
            "inversesquared": self._inverse_squared_inversed,  # class name since statsmodels 0.14
            "identity": self._identity_inversed,
            "log": self._log_inversed,
            "cloglog": self._cloglog_inversed,
            "logc": self._logc_inversed,
            "loglog": self._loglog_inversed,
...
class SklearnGLMModelAssembler(GLMMixin, SklearnLinearModelAssembler):
    ...
    def _get_link_function_name(self):
        return type(self.model._base_loss.link).__name__
